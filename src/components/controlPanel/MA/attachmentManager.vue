...
export default {
    ...
    async loadingList() {
      this.loading = true;
      let fileList = await getAttachmentList(this.selectFeatures[0].get("id")).catch(err => console.log(err));
      if (typeof fileList === 'string' || ('Error' in fileList && (fileList = fileList.Msg))){
