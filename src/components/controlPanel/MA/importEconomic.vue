  ...
  >
    <div class="import_content">
      <el-row type="flex" style="margin-bottom: 5px;">
        <div style="margin: 4px 3px;" class="label">数据年份:</div>
        <div style="margin: 0px 3px;">
          <el-input
            placeholder="请输入年份"
            suffix-icon="el-icon-date"
            v-model.number="yearImport"
            :class="{attention: yearImport == ''}"
            size="mini"
          ></el-input>
        </div>
      </el-row>
      <el-upload
        ...
        ref="upload"
        drag
        action="./dataPages/ma/execl.aspx"
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip" style="color: #909399;">
          <el-tag size="small">
            <el-link style="font-size: 12px;" :underline="false" href>示例文件下载</el-link>
          </el-tag>
          <br />请先输入年份, 方可导入。
          <br />导入数据时出现不是预期格式时, 请另存为office2007版本或用wps保存
        </div>
      </el-upload>
    </div>
...
import { importExcelForJJ } from "@/data/layerExcelIO";
...
export default {
  ...
  methods: {
    ...
    uploadExcel() {
      ...
      formData.append('Year', this.yearImport)
      this.uploading = true;
      importExcelForJJ(formData)
        .then(res => {
          this.uploading = false;
          this.$refs.upload.clearFiles();
          this.ishasFile = false;
          let msg = `文件上传成功! 成功${res.success}条, 更新${
            res.update
          }条, 错误${res.count - res.success - res.update}条`;
          msg +=
            res.errorString == ""
              ? ""
              : `<div style="color: #F56C6C; max-height: 100px; width: 300px; overflow: scroll;">错误信息: <br/>${res.errorString.replace(
                  /\n/g,
                  "。<br/>"
                )}</div>`;
