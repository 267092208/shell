...
export default {
  ...
  methods: {
    ...
    selectRow(row, oldRow) {
     ...
     async EditEnd(row) {
      if (this.editMark != null) {
        ...
        model['uid'] = row['ID']
        const res = await layerData.updateItemForRk(row['ID'], model)
      }
    },
    dropdownClick(e){
    ...
    async createRanking() {
      this.creating = true;
      const result = await createRankingByYear(
        this.selectFeatures[0].get("id"),
        this.yearImport
      );
      this.tableData = await getRankingDataWithYear(
        this.selectFeatures[0].get("id"),
        this.yearImport.toString()
      );
      this.yearOptions = await getRankingYearList(this.selectFeatures[0].get("id"));
    ...
    async openDiaglog() {
      ...
      let fields = await getRankingFields();
      this.fields = fields
        .map(item => ({
