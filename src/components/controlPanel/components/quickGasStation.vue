<template>
  <div class="ctl-panel-container">
          ...
          <el-scrollbar style=" height: 125px; width: 100%;">
            <div class="tab-panel-group">
              <el-row type="flex">
                ...
                <el-row
                  ...
                  align="center"
                  class="icon-btn"
                  @click.native="importLayerForQuickVisible = true"
                >
                  ...
                  ></el-avatar>
                  <!-- <div class="quickImportTip" ref="quickImportTip">高速专有格式</div> -->
                </el-row>
                <el-row
                  type="flex"
                  justify="center"
            ...
            <div class="tab-panel-group">
              <el-row type="flex" class="labelTable">
                <el-col v-for="(item,index) in statusList" :key="index" class="label-btn-wrap">
                  <div class="statusLabel">{{item.name}}</div>
                  <el-switch
                    class="statusSW"
                    v-model="item.visible"
                    active-color="#13ce66"
                    @change="changeStatus(item.name, item.visible, 'gsyz')"
                  ></el-switch>
                </el-col>
                  ...
                  <el-dropdown class="label-dropdown">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item
                        ...
                        :key="index"
                        @click.native="choiceScale(item, 'gsyz')"
    ...
    <import-layer-for-quick
      :dialogVisible.sync="importLayerForQuickVisible"
      exampleXlsPath="./dataPages/gsyz/gsyz-import-test.xlsx"
    ></import-layer-for-quick>
...
import importLayerForQuick from "@/components/controlPanel/common/importLayerForQuick.vue";
...
export default {
  ...
  data() {
    return {
      importLayerForQuickVisible: false
    };
  },
  components: {
    importLayerForQuick
  ...
  methods: {
    // hoverQuickImport(e) {
    //   this.$refs.quickImportTip.style.background = "#d4d4d4";
    // },
    // outQuickImport(e) {
    //   this.$refs.quickImportTip.style.background = "#fff";
    // },
    /* end 面板ui操作 */
    /**
     * 图层标签
    ...
    initLabel() {
      const labels = [
        ...
        { name: "显示编号", renderer: null, visible: false },
        // { name: "VA油站", renderer: null, visible: false },
     ...
     */
    init() {
      let layer = this.base.find(item => item.id === "gsyz");
      this.statusList = layer.extLayers || this.initLabel(); // FIXME: 暂时使用
