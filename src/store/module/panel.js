  ...
  store.watch(
    ...
    (newval,oldval) => {
      ...
      if (newval) {
        ...
        console.log(rightPanelName)
        if (rightPanelName != 'commPropertyPanel' && typeof extent === 'object' && 'editPanel' in extent && extent['editPanel'] === true) { // 通过select取消的不包裹通用属性面板(编辑状态）
         // don't not anything
        } else {
